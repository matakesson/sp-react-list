...
import { PropertyPaneButton, PropertyPaneCheckbox } from '@microsoft/sp-property-pane';
...
export default class ReactListWebPart extends BaseClientSideWebPart<IReactListWebPartProps> {
  ...
  public render(): void {
    ...
    protected onInit(): Promise<void> {
      ...
      this.themeManager = new ThemeManager(this.properties.primaryColor, this.properties.color, this.properties.backgroundColor);
  ...
  private fetchConfigsFromList(): void {
    ...
    Web([sp.web, siteUrl]).lists.getByTitle(listName).items.select(this.properties.valueProp, this.properties.nameProp)().then(items => {
      this.setConfigsFromList(items);
    }).catch(error => {
      console.error("Error fetching list items:", error);
      alert('Error fetching list items. Please check the Site URL and List Name.');
    });
  }
  ...
  private setConfigsFromList(items: any[]): void {
    items.forEach(item => {
      ...
      this.properties[item[this.properties.nameProp] as string] = item[this.properties.valueProp];
    });
